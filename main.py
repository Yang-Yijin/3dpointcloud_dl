...
def train(args):  # 定义训练函数，接受参数args
    ...
    for epoch in range(args.epochs):  # 训练循环，遍历每个epoch
        ...
        with torch.no_grad():  # 不计算梯度
            ...
            writer.add_scalar("train/balance_train_acc",balance_train_acc, epoch)
            writer.add_scalar("val/val_acc",val_acc, epoch)
            writer.add_scalar("val/balance_val_acc",avg_per_class_acc, epoch)
...
if __name__ == "__main__":
    ...
    #     log_path = os.path.join('./log', 'DGCNN_' + p["optimizer"] + "_lr:" + str(p["lr"]) + "_heads:" + str(p["att_heads"])+"_bupingheng")
    #     writer = SummaryWriter(log_path)
    #     params = Params(model=DGCNN, epochs=60, num_points=1024, emb_dims=1024, k=20, optimizer=p['optimizer'],
    #                     lr=p['lr'], att_heads=p['att_heads'], momentum=0.9, dropout=0.5, dump_file=True, dry_run=False)
    #     train(params, writer)
    #     test(params, writer)

    #     for p in hyperparams:
    #         log_path = os.path.join('./log', 'DGCNN_' + p["optimizer"] + "_lr:" + str(p["lr"]) + "_heads:" + str(p["att_heads"])+"_k:20_dropout:0.3")
    #         writer = SummaryWriter(log_path)
    #         params = Params(model=DGCNN, epochs=60, num_points=1024, emb_dims=1024, k=20, optimizer=p['optimizer'],
    #                         lr=p['lr'], att_heads=p['att_heads'], momentum=0.9, dropout=0.3, dump_file=True, dry_run=False)
    #         train(params, writer)
    #         test(params, writer)

    for p in hyperparams:
        log_path = os.path.join('./log',
                                'MADGCNN_k_15' + p["optimizer"] + "_lr:" + str(p["lr"]) + "_heads:" + str(
                                    p["att_heads"]) + "bupinghengshujuji")
        writer = SummaryWriter(log_path)
        params = Params(model=MADGCNN, epochs=120, num_points=1024, emb_dims=1024, k=15, optimizer=p['optimizer'],
                        lr=p['lr'], att_heads=p['att_heads'], momentum=0.9, dropout=0.5, dump_file=True, dry_run=False)
        train(params, writer)
        test(params, writer)